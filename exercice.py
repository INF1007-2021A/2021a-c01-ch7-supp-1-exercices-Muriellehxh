...
def get_fibonacci_number(f):
    if f==0:
        ...
        return get_fibonacci_number(f-1) + get_fibonacci_number(f-2)
...
def get_fibonacci_sequence(length, seq = [0, 1]):
    if length <= 2:
        ...
        # we get the last element with seq[-1], and the element right before it with seq[-2]
    else:
        return seq
...
def get_sorted_dict_by_decimals(dictionary):
    ...
    return dict(sorted(dictionary.items(), key=lambda x: x[1] % 1.0))
...
if __name__ == "__main__":
    ...
    for fibo_num in get_fibonacci_number(10):
        print(fibo_num, end=" ")
    print("\n")
